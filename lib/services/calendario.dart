...
class _CalendarioState extends State<Calendario> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _loadUserId().then((_) {
      _loadItemsForMonth(_focusedDay); // <--- Cambia aquí
  ...
  Future<void> _loadItemsForDay(DateTime day) async {
    ...
    try {
      ...
      // --- AGREGA FERIADOS DE COLOMBIA ---
      final holidays = await fetchColombiaHolidays(day.year);
      final holiday = holidays.firstWhere(
        (h) => h['date'] == formattedDate,
        orElse: () => <String, dynamic>{},
      );
      if (holiday.isNotEmpty) {
        final festivoEvent = Event(
          id: 'feriado_${holiday['date']}',
          title: holiday['localName'] ?? holiday['name'] ?? 'Feriado',
          description: 'Día festivo en Colombia',
          date: formattedDate,
          startTime: '',
          endTime: '',
        );
        _selectedItems.value = [..._selectedItems.value, festivoEvent];
      }

      // Después de cargar todos los items (eventos, tareas, feriados):
      // Agrupa por fecha
      final allItems = [...tasksData, ...eventsData];
      if (holiday.isNotEmpty) {
        final festivoEvent = Event(
          id: 'feriado_${holiday['date']}',
          title: holiday['localName'] ?? holiday['name'] ?? 'Feriado',
      ...
      for (var item in allItems) {
        DateTime date;
        if (item is Task) {
          date = DateTime.parse(item.dueDate);
  ...
  Future<void> _loadItemsForMonth(DateTime month) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;
    ...
    try {
      ...
      // 2. Cargar tareas del mes
      final tasksSnapshot = await FirebaseFirestore.instance
          .collection('users')
          .doc(user.uid)
          .collection('tasks')
          .where('dueDate', isGreaterThanOrEqualTo: DateFormat('yyyy-MM-dd').format(firstDay))
          .where('dueDate', isLessThanOrEqualTo: DateFormat('yyyy-MM-dd').format(lastDay))
          .get();
      ...
      // 3. Cargar feriados del mes (solo una vez)
      final holidays = await fetchColombiaHolidays(month.year);
      ...
      for (var holiday in holidays) {
        ...
        if (date.month == month.month) {
          ...
          final key = DateTime(date.year, date.month, date.day);
          grouped.putIfAbsent(key, () => []).add(festivoEvent);
        }
      }
      ...
      // Opcional: recargar el día seleccionado
      await _loadItemsForDay(_selectedDay);

    } catch (e) {
      debugPrint('Error loading month items: $e');
    } finally {
      setState(() {
        _isLoading = false;
      });
                ...
                children: [
                  ...
                  TableCalendar(
                    ...
                    onPageChanged: (focusedDay) {
                      ...
                      _loadItemsForMonth(focusedDay);
                    },
                    eventLoader: (day) {
                      final key = DateTime(day.year, day.month, day.day);
                      // Si hay al menos un evento/tarea/feriado, devuelve una lista con un elemento
                      return (_eventsByDay[key]?.isNotEmpty ?? false) ? [1] : [];
                    },
                    calendarStyle: CalendarStyle(
                      ...
                      markerDecoration: BoxDecoration(
                        color: Colors.orange, // Puedes cambiar el color del punto
                        shape: BoxShape.circle,
                      ),
  ...
  Widget _buildEventItem(Event event, Color textColor) {
    final isFestivo = event.description == 'Día festivo en Colombia';
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            if (isFestivo) const SizedBox(width: 8),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      color: isFestivo
                          ? Colors.orange
                          : (isExpired ? Colors.grey : textColor),
                      fontSize: 14,
                      fontWeight: isFestivo ? FontWeight.bold : FontWeight.normal,
  ...
  Future<List<Map<String, dynamic>>> fetchColombiaHolidays(int year) async {
  final url = Uri.parse('https://date.nager.at/api/v3/PublicHolidays/$year/CO');
  final response = await http.get(url);

  if (response.statusCode == 200) {
    final List<dynamic> data = jsonDecode(response.body);
    return data.map<Map<String, dynamic>>((e) => {
      'date': e['date'], // formato: "2025-01-01"
